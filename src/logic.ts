...
function parseField(field: string, min: number, max: number): number[] {
  ...
  for (const part of field.split(",")) {
    ...
    else if (range.includes("-")) { const [s, e] = range.split("-").map(Number); for (let i = s; i <= e; i += step) values.add(i); }
...
function describeCron(fields: string[]): string {
  ...
  if (dow !== "*") {
    ...
    else { const days = dow.split(",").map((d) => DAY_NAMES[parseInt(d)] || d); parts.push(`on ${days.join(", ")}`); }
  }
  return parts.join(" ");
}
...
function getNextRuns(fields: string[], count: number): string[] {
  ...
  const runs: string[] = [];
  const check = new Date(Date.now() + 60000);
  for (let i = 0; i < 525600 && runs.length < count; i++) {
    const d = new Date(check.getTime() + i * 60000);
    if (minutes.includes(d.getUTCMinutes()) && hours.includes(d.getUTCHours()) &&
...
function validateCron(expression: string): { valid: boolean; error?: string; fields?: string[] } {
  ...
  if (fields.length < 5 || fields.length > 6) return { valid: false, error: `Expected 5 or 6 fields, got ${fields.length}` };
  const cron5 = fields.length === 6 ? fields.slice(1) : fields;
  for (let i = 0; i < 5; i++) {
    const values = parseField(cron5[i], FIELD_RANGES[i][0], FIELD_RANGES[i][1]);
    if (values.length === 0) return { valid: false, error: `Field '${FIELD_NAMES[i]}' (${cron5[i]}) produces no valid values` };
  }
...
export function registerRoutes(app: Hono) {
  app.post("/api/parse", async (c) => {
    ...
    if (!body?.expression) return c.json({ error: "Missing required field: expression" }, 400);
    const { valid, error, fields } = validateCron(body.expression);
    if (!valid || !fields) return c.json({ error: `Invalid cron expression: ${error}` }, 400);
    ...
    for (let i = 0; i < 5; i++) breakdown[FIELD_NAMES[i]] = { raw: fields[i], values: parseField(fields[i], FIELD_RANGES[i][0], FIELD_RANGES[i][1]) };
    return c.json({ expression: body.expression, valid: true, description, timezone: body.timezone || "UTC", nextRuns, breakdown, hasSeconds: body.expression.trim().split(/\s+/).length === 6 });
  });
}
